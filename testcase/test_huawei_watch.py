...
from pageobject.huawei_watch.main_page_huawei import First_Page_HuaweiWatch
from pageobject.huawei_watch.menu_page_huawei import menuone_Page_HuaweiWatch
from pageobject.huawei_watch.startup_page_huawei import Login_Page_HuaweiWatch


@allure.severity("critical")  #严重缺陷
@allure.epic("项目名称: 华为手表互动小说")
@allure.feature("冒烟测试")
@allure.story("用例的标题: 华为手表互动小说冒烟测试")
class Testhuawei():
    l_obj = LogcatCathcher()#获取app后台运行日志

    def setup(self) -> None:
        driver = Config()
        self.driver = driver.devices("http://127.0.0.1:4723/wd/hub",r"\yaml\huawei_watch")
    ...
    def common(self):
        startup = Login_Page_HuaweiWatch(self.driver)
    ...
    #@pytest.mark.skip()  # 跳过该条用例
    @pytest.mark.flaky(reruns=2, reruns_delay=3)  # 失败重试两次，中间间隔三秒
    @pytest.mark.smoke
    ...
    def test_home(self):
        firstpage = First_Page_HuaweiWatch(self.driver)
        menupage=menuone_Page_HuaweiWatch(self.driver)
        self.common()
        firstpage.ivMicGuide_click()
        time.sleep(20)
    ...
    def test_home3(self):
        ...
        firstpage.swipeUp(100, 17, 0.5, 0.9, 0.1)
    ...
    def test_personal(self):
        ...
        firstpage.ivMenu_click()
        pytest.assume(firstpage.isElementPresent(menupage.homepage))
    ...
    def test_personal5(self):
        ...
        pytest.assume(firstpage.isElementPresent(firstpage.ivMenu))
    ...
    def test_dialogue2(self):
        ...
        time.sleep(22)
        time.sleep(185)
        pytest.assume(firstpage.is_element_exist("推荐作品如下"))
