...
from pageobject.oppo_watch.main_page import First_Page_OppoWatch
from pageobject.oppo_watch.menu_page import menuone_Page_OppoWatch
from pageobject.oppo_watch.startup_page import Login_Page_OppoWatch


@allure.severity("critical")  #严重缺陷
@allure.epic("项目名称: oppo手表互动小说")
@allure.feature("冒烟测试")
@allure.story("用例的标题: oppo手表互动小说冒烟测试")
class Testoppo():
    l_obj = LogcatCathcher()#获取app后台运行日志

    def setup(self) -> None:
        driver = Config()
        self.driver = driver.devices("http://127.0.0.1:4723/wd/hub",r"\yaml\oppo_watch")
    ...
    def common(self):
        startup = Login_Page_OppoWatch(self.driver)
    ...
    def test_login5(self):
        startup = Login_Page_OppoWatch(self.driver)
        firstpage = First_Page_OppoWatch(self.driver)
        startup.cbAgreement_click()
        startup.tvEnter_click()
    ...
    def test_access1(self):
        ...
        pytest.assume(startup.isElementPresent(firstpage.ivMicGuide))
        pytest.assume(startup.ImageView_pop())
        startup.save_screenshot('进入新手引导且提示麦克风权限被拒绝')
    ...
    def test_home(self):
        firstpage = First_Page_OppoWatch(self.driver)
        menupage=menuone_Page_OppoWatch(self.driver)
        self.common()
        firstpage.ivMicGuide_click()
        time.sleep(20)
